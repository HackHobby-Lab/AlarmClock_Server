...
void checkBridgeConnection() {
  ...
  if (storedUsername.length() > 0) {
    if (BridgeConnection(storedBridgeIP, storedUsername)) {
      ...
      // Username exists, but bridge is not reachable
      Serial.println("Bridge not reachable.");
      server.send(400, "text/plain", "Bridge not reachable");
    } 
  } else {
